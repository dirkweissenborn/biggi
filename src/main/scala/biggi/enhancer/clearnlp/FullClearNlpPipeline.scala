package biggi.enhancer.clearnlp
...
import biggi.model.annotation.SemanticRoleLabel
...
class FullClearNlpPipeline(val dictionaryFile:File,
    ...
    private val components: Array[AbstractComponent] =
        if(onlyParse)
            Array(tagger, analyzer, parser)
        else
            Array(tagger, analyzer, parser, identifier, classifier, labeler)
...
object FullClearNlpPipeline{
    ...
    def fromConfiguration(properties:Properties, onlyDep:Boolean = false):FullClearNlpPipeline = {
        if (enhancer == null) {
