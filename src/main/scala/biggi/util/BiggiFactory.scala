...
object BiggiFactory {
    ...
    def initGraph(g:TitanGraph) {
        ...
        g.makeType().name("semtypes").dataType(classOf[String]).indexed("search",classOf[Vertex]).makePropertyKey()
        g.makeType().name("uttIds").dataType(classOf[String]).indexed("search",classOf[Edge]).makePropertyKey()
